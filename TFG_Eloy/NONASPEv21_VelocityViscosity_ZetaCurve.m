%==========================================================================
% NONASPEv21_VelocityViscosity_ZetaCurve
%
% ------------
% Description:
% ------------
%
% The code can operate in three modes:
%   - 'VelVisco': obtains velocity and viscosity profiles for a fixed n
%   - 'ZetaCurve': computes ζ_c as a function of n (laminar/turbulent)
%   - 'EtaxCurve': evaluates dη/dx versus n
%
%
% NONASPE                                                                 
...
%   - Range of values for ZetaCurve and EtaxCurve cases
if strcmp(mode, 'ZetaCurve') || strcmp(mode, 'EtaxCurve')
    n_val = 0.4:0.05:1.8;
end
%   - Single value for VelVisco case
if strcmp(mode, 'VelVisco')
    n_val = 1.4;
end

% Initialization of values for the ZetaCurve case
if strcmp(mode, 'ZetaCurve')
    zeta_c_lam = zeros(size(n_val));
    zeta_c_turb = zeros(size(n_val));
end
% Initialization of values for the EtaxCurve case
if strcmp(mode, 'EtaxCurve')
    etax_lam = zeros(size(n_val));
    etax_turb = zeros(size(n_val));
end
...
for i = 1:length(n_val)
    ...
    for j = 1:2
        ...
        if strcmp(mode, 'ZetaCurve')
            if strcmp( flow, 'laminar')
                zeta_c_lam(i) = tauw ./ (rho * g * etax * H);
            elseif strcmp( flow, 'turbulent')
                zeta_c_turb(i) = tauw ./ (rho * g * etax * H);
            end
        end
...
if strcmp(mode, 'ZetaCurve')
    figure
    % Laminar flow
    plot( n_val, zeta_c_lam, 'r-' )
    grid on
    set( gcf, 'color', 'w' )
    xlabel( '$n$', 'interpreter', 'latex' )
    ylabel( '$\zeta_c$', 'interpreter', 'latex' )
    hold on
    % Turbulent flow
    plot( n_val, zeta_c_turb, 'b-' )
    grid on
    set( gcf, 'color', 'w' )
...
if strcmp(mode, 'EtaxCurve')
    figure
    % Laminar flow
    plot( n_val, etax_lam, 'r-' )
    grid on
    set( gcf, 'color', 'w' )
    xlabel( '$n$', 'interpreter', 'latex' )
    ylabel( '$\partial \eta / \partial x$', 'interpreter', 'latex' )
    hold on
    % Turbulent flow
    plot( n_val, etax_turb, 'b-' )
    grid on
    set( gcf, 'color', 'w' )
    legend('laminar','turbulent')
    hold off
...
if strcmp(mode, 'VelVisco')
    ...
    legend('laminar','turbulent')
    hold off
    title(['Velocity profiles for n = ', num2str(n)])
    ...
    grid on
    set( gcf, 'color', 'w' )
    xlabel('$\nu_{t} [m^2s^{-1}]$','interpreter','latex')
    ylabel('$z/H$','interpreter','latex')
    % Non-Newtonian viscosity
    subplot(1,3,2)
    plot(kn(uprom, dUdz, z),  z ./ H,  'm-')
    grid on
    set( gcf, 'color', 'w' )
    xlabel('$\nu_{NN} [m^2s^{-1}]$','interpreter','latex')
    ylabel('$z/H$','interpreter','latex')
    title(['Viscosity for n = ', num2str(n)])
    % Mean effective viscosity
    subplot(1,3,3)
    plot(kavg(dUdz, z),  z ./ H,  'g-')
    grid on
    set( gcf, 'color', 'w' )
    xlabel('$\overline{\nu} [m^2s^{-1}]$','interpreter','latex')
    ylabel('$z/H$','interpreter','latex')
end
