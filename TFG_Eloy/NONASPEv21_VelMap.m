...
% ------------
%
% It is necessary to select either 'laminar' or 'turbulent' flow by setting the variable 'flow' before running the simulation
%
% The code can operate in one mode:
%   - 'VelMap': produce contour maps of normalized velocity (u/|u_max|)
%   
%       
%
% NONASPE                                                                 
...
%   - Range of n values forVel Map cases
if strcmp(mode, 'VelMap') 
    n_val = 0.4:0.05:1.8;
end

% Initialization of values for the VelMap case
if strcmp(mode, 'VelMap')
    x = n_val;               % n
    y = zeros(size(H / N));  % H/N
    V = [];                  % u/u_max
end
...
if strcmp(mode, 'VelMap')
    figure
    contourf(X, Y, V);
    zlabel( '$u/|u_{\mathrm{max.}}|$', 'interpreter', 'latex' )
    ylabel( '$z/H$', 'interpreter', 'latex' )
    xlabel( '$n$', 'interpreter', 'latex' )
    cb = colorbar('eastoutside');
    cb.Label.String = '$u/|u_{\mathrm{max.}}|$';
