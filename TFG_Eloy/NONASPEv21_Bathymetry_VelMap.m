%==========================================================================
% NONASPEv21_Bathymetry_VelMap
%
% ------------
% Description:
% ------------
%
% This model simulates the velocity distribution in a semi-enclosed basin of
% Ligeia Mare, one of Titan’s largest hydrocarbon seas. It evaluates the 
% effect of different rheological indices (n) on the flow dynamics and 
% incorporates the actual bathymetric profile of the basin.
%
% It computes the fluid column for each discrtization point in the
% x-domain.
%       
%
% NONASPE                                                                 
...
U_matrix = zeros(N, length(x_dom));
Z_matrix = zeros(N, length(x_dom));
...
for j = 1:length(x_dom)
    ...
    % Store results as columns in the result matrices
    U_matrix(:,j) = U;
    Z_matrix(:,j) = z;
end
...
% Plotting
figure
surf(X, Z, U_matrix, 'EdgeColor', 'none')
shading interp
colormap turbo
colorbar
...
title(['Velocity distribution for n = ' num2str(n_val)], 'Interpreter', 'latex')
set(gcf, 'Color', 'w')
view([0, 90])
